...
static uint32_t hash_data(uint64_t *block, size_t size)
{
    uint64_t hash = (*block ^ 2166136261U) * 0xBF58476D1CE4E5B9;
	...
	switch (size & 7)
	{
        case 7: last |= (uint64_t)data[7] << 56; /* fallthrough */
        case 6: last |= (uint64_t)data[6] << 48; /* fallthrough */
        case 5: last |= (uint64_t)data[5] << 40; /* fallthrough */
        case 4: last |= (uint64_t)data[4] << 32; /* fallthrough */
        case 3: last |= (uint64_t)data[3] << 24; /* fallthrough */
        case 2: last |= (uint64_t)data[2] << 16; /* fallthrough */
        case 1: last |= (uint64_t)data[1] << 8; /* fallthrough */
        case 0: last |= (uint64_t)data[0] << 0; /* fallthrough */
	}
...
#define GIANT_STEP 0x20
#define BABY_RANGE 0x8000000
#define HASHMAP_SIZE 147639491UL
...
struct HashMapPre 
{
    struct NodePre buckets[HASHMAP_SIZE];
};

static void hashmap_init(struct HashMapPre *map)
{
    uint32_t i;
    ...
    for (i = 0; i < HASHMAP_SIZE; i++)
    {
        ...
        map->buckets[i].point = (uint64_t *)malloc(10 * sizeof(uint64_t));
        map->buckets[i].array = (uint32_t *)malloc(10 * sizeof(uint32_t));
    }
}

static void hashmap_set(struct HashMapPre *map, uint64_t *key, size_t ksize, uint32_t value)
{
    uint32_t index = hash_data(key, ksize) % HASHMAP_SIZE; 

    map->buckets[index].array[map->buckets[index].length] = value;
    map->buckets[index].point[map->buckets[index].length] = *key;
    map->buckets[index].length += 1;
}

static int hashmap_get(struct HashMap *map, uint64_t *key, size_t ksize, uint32_t *value)
{
    uint32_t index = hash_data(key, ksize) % HASHMAP_SIZE; 

    uint32_t node_index = map->buckets[index] >> 5;
    uint32_t node_len = map->buckets[index] & 0x1F;

    for (index = node_index; index < node_index + node_len; index ++)
    {
        if (*key == map->nodes[index].point)
        {
            *value = map->nodes[index].head;
            return 1;
        }
    }
...
static void hashmap_free(struct HashMapPre *map)
{
	uint32_t i;
    ...
    for (i = 0; i < HASHMAP_SIZE; i++)
	{
        ...
        free(map->buckets[i].point);
        map->buckets[i].array = NULL;
        map->buckets[i].point = NULL;
    }
}
...
/* LIFTED ELGAMAL EXTENSION FROM HERE */


SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_build_discrete_log(
    const secp256k1_context *ctx,
    struct HashMapPre *table
) SECP256K1_ARG_NONNULL(1) SECP256K1_ARG_NONNULL(2);


SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_load_discrete_log(
    const secp256k1_context *ctx
) SECP256K1_ARG_NONNULL(1);


SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_elgamal_decryption(
	const secp256k1_context* ctx, 
